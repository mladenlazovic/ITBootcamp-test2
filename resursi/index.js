let dinos = [
    {
        id: 0,
        name: 'Scipionyx',
        img: 'http://images.dinosaurpictures.org/Scipionyx_QY_200_3742.jpg',
        cena: 221
    },
    {
        id: 1,
        name: 'Becklespinax',
        img: 'http://images.dinosaurpictures.org/altispinax-becklespinax_e05c.jpg',
        cena: 275
    },
    {
        id: 2,
        name: 'Sciurumimus',
        img: 'http://images.dinosaurpictures.org/Sciurumimus_albersdoerferi_web_fa15.jpg',
        cena: 341
    },
    {
        id: 3,
        name: 'Hypsilophodon',
        img: 'http://images.dinosaurpictures.org/hypsilophodon-1024x636_6963.jpg',
        cena: 189
    },
    {
        id: 4,
        name: 'Dacentrurus',
        img: 'http://images.dinosaurpictures.org/Dacentrurus_20140118_5d27.jpg',
        cena: 315
    },
    {
        id: 5,
        name: 'Iguanodon',
        img: 'http://images.dinosaurpictures.org/Iguanodon_1157950_ea00.jpg',
        cena: 374
    },
    {
        id: 6,
        name: 'Asylosaurus',
        img: 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ae/Asylosaurus_NT.jpg/440px-Asylosaurus_NT.jpg',
        cena: 301
    },
    {
        id: 7,
        name: 'Efraasia',
        img: 'http://images.dinosaurpictures.org/efraasia_0a4e.jpg',
        cena: 199
    },
    {
        id: 8,
        name: 'Cryptosaurus',
        img: 'https://images.dinosaurpictures.org/Cryptosaurus/Cryptosaurus_tumblr_inline_on3a5nVchm1rx4yme_1280_94c8.jpg',
        cena: 218
    }
]


const divForm = document.querySelector('.form-style-2')
const forma = document.querySelector('#forma')
const inputText = document.querySelector('.input-field')
const select = document.querySelector('.select-field')
const napomena = document.querySelector('.textarea-field')
const btnIspis = document.querySelector('#btn-all')
const divItemCont = document.querySelector('#item-container')

let nizDino = []

divForm.addEventListener('submit', (event) =>{
    event.preventDefault()

    if(inputText.value.length != 0 && inputText.value.length >= 4){
        let divInfo = document.createElement('div')
        divInfo.className = 'item'
        let pKupac = document.createElement('p')
        pKupac.textContent = `Купац: ${inputText.value}`
        let pNapomena = document.createElement('p')
        pNapomena.textContent = `Напомена: ${napomena.value}`
        let pDinosaurus = document.createElement('p')
        pDinosaurus.textContent = `Диносаурус: ${select.value}`
        let pCena = document.createElement('p')
        pCena.textContent = `Цена: 0`
    
        const btnDelete = document.createElement('button')
        btnDelete.textContent = 'Обриши'
    
        btnDelete.addEventListener('click', () =>{
            divInfo.remove()
        })
    
        divItemCont.appendChild(divInfo)
        divInfo.append(pKupac,pNapomena, pDinosaurus,pCena, btnDelete)
    
        nizDino.push(divInfo)
        
    
        btnIspis.addEventListener('click', () =>{
            console.log(divInfo);
        })
    }
    else{
        console.log('Polje kupac ne sme biti prazno ili ispis kraci od 4 karaktera');
    }

   
})
